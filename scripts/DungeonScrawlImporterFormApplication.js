export class DungeonScrawlImporterFormApplication extends FormApplication {
    ...
    async parseShapes(shapes, doorIDs, stairsIds, gridCellSize, offsetX, offsetY) {
        ...
        let sceneX = sceneDimensions.sceneRect ? sceneDimensions.sceneRect.x : sceneDimensions.sceneX;
        let sceneY = sceneDimensions.sceneRect ? sceneDimensions.sceneRect.y : sceneDimensions.sceneY;

        let calculatedXOffset = sceneCellSize * offsetX + mostNegativeX * -1 + sceneX;
        let calculatedYOffset = sceneCellSize * offsetY + mostNegativeY * -1 + sceneY;

        uniqueWallData = uniqueWallData.map(value => {
            value.c[0] = value.c[0] + calculatedXOffset
            value.c[1] = value.c[1] + calculatedYOffset
            value.c[2] = value.c[2] + calculatedXOffset
            value.c[3] = value.c[3] + calculatedYOffset
            return value
        })
        // noinspection JSUnresolvedFunction
        await game.scenes.current.createEmbeddedDocuments("Wall", uniqueWallData)

        return {xOffset: calculatedXOffset, yOffset: calculatedYOffset}
    }
    ...
    getLights(dataNodes) {
        let lightIDs = []
        for (let node in dataNodes) {
            if (dataNodes[node].type === "LIGHT") lightIDs.push(dataNodes[node])
        }
        return lightIDs
    }

    /**
     * @param {Array} lightData
     * @param {{xOffset: number, yOffset: number}} offsetData
     * @param {number} gridCellSize
     */
    async processLights(lightData, offsetData, gridCellSize) {
        let lights      = []
        let data        = []
        let lightType   = ''
        let lightConfig = {}
        ...
        for (let key in lightData) {
            ...
            let light = {
                ...
                config: {
                    ...
                    color:      "#" + lightConfig.shared.colour.colour.toString(16).padStart(6, "0"),
                    dim:        lightConfig[lightType].radius,
                    hidden:     !data.visible,
                    luminosity: lightConfig[lightType].strength,
                }
            }
            lights.push(light)
        }

        await currentScene.createEmbeddedDocuments("AmbientLight", lights)
    }
